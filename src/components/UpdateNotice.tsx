...
export default function UpdateNotice() {
  const [showKnockoutUpdate, setShowKnockoutUpdate] = useState(true);
  const [showMatchFeaturesUpdate, setShowMatchFeaturesUpdate] =
    useState(true);

  if (!showKnockoutUpdate && !showMatchFeaturesUpdate) {
    return null;
  }
      ...
      {showKnockoutUpdate && (
        <section className="rounded-3xl border border-red-400/40 bg-red-500/15 p-5 shadow-xl">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm font-bold leading-6 text-white/80">
                ...
                penalties. Your game score prediction is for the 90 minutes or
                120 minutes if the game had extra time.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={() => setShowKnockoutUpdate(false)}
          ...
          <ul className="mt-4 grid gap-2 text-sm text-white/80 sm:grid-cols-3">
            ...
            <li className="rounded-xl border border-red-300/20 bg-red-400/10 p-3 font-bold">
              Select advancing team using radio buttons after selecting the
              score!
            </li>
          </ul>
        </section>
